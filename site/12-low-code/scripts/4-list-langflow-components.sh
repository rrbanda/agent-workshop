#!/bin/bash
# List components in a Langflow flow via REST API
...
# List components (nodes) in the flow
echo "Components in flow:"
echo ""

echo "$RESPONSE" | jq -r '
    .data.nodes[] |
    "  Type: \(.data.type // .type)
   Display Name: \(.data.node.display_name // "N/A")
   ID: \(.id)
   ---"
'

# Count components
COMPONENT_COUNT=$(echo "$RESPONSE" | jq '.data.nodes | length')
echo ""

"""LangGraph agent calling Finance MCP for Lone Pine orders."""
...
async def main():
    ...
    async def call_tool(state: State) -> State:
        # Search for orders by customer_id (LONEP = Lonesome Pine Restaurant)
        result = await tool.ainvoke({"customer_id": "LONEP"})
    ...
    out = await graph.ainvoke({"user_input": "find orders for LONEP"})
    print("\nRaw Result:\n", out["tool_result"])

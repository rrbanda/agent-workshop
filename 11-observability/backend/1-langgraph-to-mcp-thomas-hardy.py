"""LangGraph agent calling Customer MCP for Thomas Hardy customer query."""
...
async def main():
    ...
    async def call_tool(state: State) -> State:
        ...
        # For "who does Thomas Hardy work for?" we search by contact_name
        result = await tool.ainvoke({"contact_name": "Thomas Hardy"})
    ...
    out = await graph.ainvoke({"user_input": "who does Thomas Hardy work for?"})
    print("\nRaw Result:\n", out["tool_result"])

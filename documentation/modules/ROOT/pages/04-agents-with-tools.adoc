...
* xref:02-mcp-servers.adoc[Module 02: MCP Servers] running (ports {customer-mcp-port} and {finance-mcp-port})
...
|*Most common cause:* Your Llama Stack is remote but your MCP servers are on `localhost`. A remote Llama Stack cannot reach `localhost` on your machine. Either deploy MCP servers remotely (instructor-led) or run Llama Stack locally. Check Module 00's "Deployment Scenarios" section.

|Empty tool list
|Verify MCP servers are running on ports {customer-mcp-port}/{finance-mcp-port}

|Connection refused
|Check `LLAMA_STACK_BASE_URL` and that the Llama Stack server is running

|Agent doesn't call tools
|Ensure `INFERENCE_MODEL` supports tool calling (check your Llama Stack server's available models)
|===

IMPORTANT: *Remote Llama Stack + local MCP does not work.* If your Llama Stack runs on OpenShift (remote) and your MCP servers run on `localhost`, the Llama Stack server cannot reach them. You will get `RuntimeError: No response available` with no further explanation. See xref:00-setup.adoc#_deployment_scenarios[Deployment Scenarios] for the correct configuration.

= Module 03: Llama Stack Basics
include::_attributes.adoc[]

== Learning Objectives

* Create a basic Llama Stack agent (hello world)
* Understand streaming vs non-streaming responses
* Inspect registered toolgroups and tools
* Learn the key Llama Stack SDK classes: `LlamaStackClient`, `Agent`, `Client`

TIP: *Capstone Preview:* In the capstone, you will use these same `Agent` and `AgentEventLogger` patterns to create a Mortgage Approval Agent that reasons over real financial data.

== Prerequisites

* xref:00-setup.adoc[Module 00: Environment Setup] completed
* xref:02-mcp-servers.adoc[Module 02: MCP Servers] deployed and registered (required for scripts 2 and 3 which list MCP tools)
* Llama Stack server running

== Concepts

*{llama-stack-repo}[Llama Stack,window=_blank]* is an open-source, community-driven platform for building AI applications. It provides a unified API for inference, agents, tools, RAG, safety, and evals with a plugin architecture that supports providers like Ollama, vLLM, AWS Bedrock, and others. The `Agent` class provides a high-level API for creating agents with tools, while `LlamaStackClient` provides low-level access to individual APIs (models, tools, safety, etc.).

== Scripts

[cols="2,4", options="header"]
|===
|Script
|What It Does

|`1_hello_world_agent_no_stream.py`
|Create an agent and get a single response

|`1_hello_world_agent_streaming.py`
|Same agent with streaming response events

|`2_list_tools.py`
|List all registered toolgroups

|`3_list_customer_tools.py`
|Inspect customer MCP tools in detail

|`3_list_finance_tools.py`
|Inspect finance MCP tools in detail
|===

== Step-by-Step

NOTE: *Working directory:* All commands in this module run from `03-llama-stack-basics/`. +
*Services needed:* Llama Stack server.

=== 1. Run Hello World (Non-Streaming)

[source,bash]
----
python 1_hello_world_agent_no_stream.py
----

This creates an agent, opens a session, and sends a single message.

=== 2. Run Hello World (Streaming)

[source,bash]
----
python 1_hello_world_agent_streaming.py
----

Observe the difference: events stream in real-time via `AgentEventLogger`.

=== 3. List Registered Tools

[source,bash]
----
python 2_list_tools.py
python 3_list_customer_tools.py
python 3_list_finance_tools.py
----

NOTE: Scripts 2 and 3 list MCP tools that must be registered with Llama Stack first. If you see empty results, complete the MCP server registration steps in xref:02-mcp-servers.adoc[Module 02 examples] before running these scripts.

== What You Should See

=== Hello World (script 1 -- non-streaming)

[source,text]
----
Base URL: <your LLAMA_STACK_BASE_URL>
Model: vllm-inference/gpt-oss-120b
An AI agent is a software system that perceives its environment...
----

The last line is the agent's answer to "Explain what an AI agent is in one sentence." (Exact text varies by model.)

=== List Tools (script 2)

[source,text]
----
==================================================
Registered Toolgroups
==================================================
Toolgroup ID:  customer_mcp
Provider ID:   ...
MCP Endpoint:  <your CUSTOMER_MCP_SERVER_URL>
--------------------------------------------------
Toolgroup ID:  finance_mcp
Provider ID:   ...
MCP Endpoint:  <your FINANCE_MCP_SERVER_URL>
--------------------------------------------------
Total toolgroups: 2
==================================================
----

=== List Customer Tools (script 3)

[source,text]
----
==================================================
Customer MCP Server Tools
==================================================
MCP Server URL: <your CUSTOMER_MCP_SERVER_URL>
Tool Name:    search_customers
Description:  Search for customers by name, email, or company
Parameters:
  - query (string)[required]: ...
--------------------------------------------------
Total tools: 2
==================================================
----

== Verification

All scripts should run without errors and produce meaningful output.

== Key Takeaways

* `LlamaStackClient` is the low-level client; `Agent` is the high-level agent wrapper
* Agents use sessions (`create_session`) and turns (`create_turn`) to manage conversations
* Streaming responses provide real-time feedback via event logging
* Tools must be registered with Llama Stack before agents can use them

== Troubleshooting

[cols="2,3", options="header"]
|===
|Problem
|Solution

|Connection refused
|Verify `LLAMA_STACK_BASE_URL` is correct and the server is running

|Empty tool lists (scripts 2, 3)
|Register MCP tools first -- see xref:02-mcp-servers.adoc[Module 02]

|"Model not found"
|Check `INFERENCE_MODEL` matches a model available on your Llama Stack server

|Import errors
|Run `pip install -r requirements.txt` from the repo root
|===

== Next Module

Proceed to xref:04-agents-with-tools.adoc[Module 04: Agents with MCP Tools] to give your agents real tools to call.

...
. *Checks application status* (via `@client_tool` functions) to see what conditions are outstanding
...
NOTE: *Working directory:* All commands run from the *repo root* (`agent-workshop/`). +
...
TIP: *Recognize the pattern:* This API follows the same Spring Boot structure you deployed in Module 01. Compare `mortgage-api/src/` with `customer-api/src/` -- same entity/repository/service/controller layers, same `data.sql` seed data approach.
...
NOTE: The capstone agent scripts use *client-side tools* (`@client_tool` in `mortgage_client_tools.py`) that call the Mortgage API directly via HTTP. The MCP server (`mortgage-mcp/`) is included for completeness and can be used for custom experiments, but the provided scripts do not require it.
...
|Mortgage API errors
|Verify the Mortgage API pod is running on OpenShift (`oc get pods -l app=mortgage-api`) and `MORTGAGE_API_BASE_URL` in `.env` is correct.

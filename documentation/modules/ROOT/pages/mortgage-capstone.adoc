...
flowchart LR
    subgraph inputs [Agent Reads]
        ...
        Tools[Application Data via Client Tools]
        Credit[Credit Reports via Client Tools]
    end
...
. *Checks application status* (via client tools) to see what conditions are outstanding
...
|Agent creation, client tool binding
...
flowchart TD
    ...
    API --> DB[("PostgreSQL - acme_mortgage")]
...
TIP: *Try it yourself:* Write your own query that asks about VA loan document requirements. Does the agent use file_search, client tools, or both? Watch the tool calls in the output to see the agent's reasoning.
...
|Mortgage API errors
|Verify PostgreSQL database `acme_mortgage` exists and API is running on port {mortgage-api-port}

|Tool calls fail
|Verify `MORTGAGE_API_BASE_URL` in `.env` points to a running Mortgage API. The capstone uses client-side tools that call the API directly (not via MCP).
...
|Ensure the shield is registered (run Module 09, script `4_register_shield.py`) and `SHIELD_ID` is set in `.env`

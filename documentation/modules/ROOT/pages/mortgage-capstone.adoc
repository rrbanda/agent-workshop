...
TIP: *Instructor-led workshop?* If your instructor has pre-deployed the Mortgage API and MCP server on OpenShift, skip the database creation, API start, and MCP server start steps below. Use the `MORTGAGE_API_BASE_URL` and `MORTGAGE_MCP_SERVER_URL` URLs provided by your instructor in your `.env` file and proceed directly to "Create the vector store."

...
TIP: If you see `"customer-api" already exists`, the build config was already created. You can skip `oc new-build` and just re-run the `cp`, `oc start-build`, and `rm` commands.
...
TIP: Same as above -- if `"finance-api" already exists`, skip `oc new-build` and re-run only the build.
